			...
			<view class="box" :style="{ '--height-bar': `${navBarHeight}px` }">
				...
				<view class="mains">
					...
					<view class="box-contt">
						<view class="list">
							<view class="content">
								<span>{{ scoketData.hasChargePower }}kWh</span>
								<span>已充电量</span>
							</view>
...
export default {
	...
	onLoad(option) {
		...
		if (option.ordernumber) {
			_this.getOrderDetail(option.ordernumber);
		} else {
			uni.getStorage({
				key: "openid",
				success(res) {
			...
			chargeing(data).then(res => {
				if (res.code == 0) {
					if (res.data) {
						...
						uni.showModal({
							title: '提示',
							content: '存在充电中订单,是否查看?',
							success: tipsRes => {
								if (tipsRes.confirm) {
									this.lookPower(res.data.parkid,res.data.id)
								} else if (tipsRes.cancel) {
									console.log('用户点击取消');
								}
							}
						})
